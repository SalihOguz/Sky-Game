...
static const float volumetricCloudWeatherMapSdfScale = _WeatherMakerWeatherMapScale.w * 0.5;
...
inline fixed4 FinalizeVolumetricCloudColor(fixed4 color, float4 uv, uint marches)
{
	...
	if (color.a >= WM_MIN_PIXEL_VALUE)
	{
		...
		// double pre-multiply to soften up the clouds more, helps remove lines
		color *= color.a;
		color *= color.a;
	}

	// ensure a min alpha
	color.a = max(color.a, WM_MIN_PIXEL_VALUE);
...
)
{
	...
	float opticalDepthMarchMultiplier = max(1.0, startOpticalDepth * _CloudRayMarchParameters.w);
	...
	float lod = startLod;
    float lodStep = 0.0;// (volumetricLod.y - startLod) * invSampleCount;
	uint inCloud = 0;
	uint maybeInCloud = 0;
			...
			if (CloudVolumetricGetCoverage(weatherData) > VOLUMETRIC_CLOUD_MINIMUM_COVERAGE_FOR_CLOUD)
			{
				...
				if (cloudSample > VOLUMETRIC_CLOUD_MIN_NOISE_VALUE)
				{
					...
					if (depth < _ProjectionParams.z)
					{
						...
						cloudSample *= saturate(CLOUD_INV_FADE * diff);
					}
