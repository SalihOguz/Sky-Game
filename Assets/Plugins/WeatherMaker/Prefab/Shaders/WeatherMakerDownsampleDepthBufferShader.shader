...
#define SAMPLE_DEPTH_4_01(source, uv) \
		float depth1 = WM_LINEAR_DEPTH_01(UNITY_SAMPLE_DEPTH(SAMPLE_DEPTH_TEXTURE(source, uv + tapsDepth1))); \
		float depth2 = WM_LINEAR_DEPTH_01(UNITY_SAMPLE_DEPTH(SAMPLE_DEPTH_TEXTURE(source, uv + tapsDepth2))); \
		float depth3 = WM_LINEAR_DEPTH_01(UNITY_SAMPLE_DEPTH(SAMPLE_DEPTH_TEXTURE(source, uv + tapsDepth3))); \
		float depth4 = WM_LINEAR_DEPTH_01(UNITY_SAMPLE_DEPTH(SAMPLE_DEPTH_TEXTURE(source, uv + tapsDepth4)));
...
#define DownsampleDepth01(source, uv) \
		SNAP_UV(uv) \
		SAMPLE_DEPTH_4_01(source, uv); \
		return (max(depth1, max(depth2, max(depth3, depth4))));
...
#define DownsampleDepth(source, uv) \
		...
		int2 position = floor(i.uv.xy * _DepthTexelSizeDest.zw); \
...
#error Invalid downsample mode
		...
		{
			WM_INSTANCE_FRAG(i);
			return WM_LINEAR_DEPTH_01(UNITY_SAMPLE_DEPTH(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, i.uv.xy)));
		}
		...
		float4 frag2(wm_full_screen_fragment_vertex_uv i) : SV_Target
		{
			...
			DownsampleDepth01(_CameraDepthTexture, i.uv.xy);
		}
