...
static const float CLOUD_INV_FADE = min(0.01, (_ProjectionParams.w * 25.0));
static const float CLOUD_COLOR_BLOCKED_BY_DEPTH_BUFFER = fixed4(0.0, 0.0, 0.0, 0.0);
...
fixed ComputeCloudAlpha(float3 rayDir, float fbm, uint cloudIndex)
{
	...
	else
	{
		...
		alpha *= alpha;
		return max(alpha, WM_MIN_PIXEL_VALUE);
	}
}
...
fixed4 ComputeCloudColor(float3 rayDir, float depth, Texture2D<float4> noiseTex,/* Texture2D<float4> maskTex,*/ float4 screenUV, uint cloudIndex, inout fixed alphaAccum)
{
	...
	{
		return CLOUD_COLOR_BLOCKED_BY_DEPTH_BUFFER;
	}
	else
	{
		...
		if (fbm < 0.005)
		{
			return fixed4(0.0, 0.0, 0.0, WM_MIN_PIXEL_VALUE);
		}
		else
		{
			// compute lighting
			...
			fixed4 finalColor = fixed4(cloudColor + (_CloudEmissionColor[cloudIndex].rgb * _CloudEmissionColor[cloudIndex].a), alpha);

			// soft fade
			UNITY_BRANCH
			if (depth < _ProjectionParams.z)
			{
				float partZ = distance(worldPos, WEATHER_MAKER_CAMERA_POS);
				float diff = (depth - partZ);
				finalColor.a *= saturate(CLOUD_INV_FADE * diff);
			}

			finalColor.a = max(WM_MIN_PIXEL_VALUE, finalColor.a);
