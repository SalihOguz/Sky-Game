...
Shader "WeatherMaker/WeatherMakerFullScreenCloudsShader"
{
	...
	SubShader
	{
		...
		inline void GetDepthAndRay(float4 uv, inout float3 rayDir, float3 forwardLine, out float depth, out float depth01)
		{
            ...
            depth01 = (_WeatherMakerVREnabled ? 1.0 : WM_SAMPLE_DEPTH_DOWNSAMPLED_TEMPORAL_REPROJECTION_01(uv.xy));
		...
		Pass
		{
			...
			inline fixed4 blendCloudTemporal(fixed4 prev, fixed4 cur, fixed4 diff, float4 uv, wm_full_screen_fragment i)
			{
				...
				if (prev.a < 0.003 || cur.a < 0.003 || diff.w >= _WeatherMakerTemporalReprojectionAlphaThreshold)
				{
					return CLOUD_COLOR_FORCE_REDRAW;
				}
                else
                {
                    ...
                    if (a1 > 0.0 && a2 > 0.0 && a3 > 0.0 && a4 > 0.0 && a5 > 0.0 && a6 > 0.0 && a7 > 0.0 && a8 > 0.0)
                    {
                        ...
                        return WM_SAMPLE_FULL_SCREEN_TEXTURE_SAMPLER(_TemporalReprojection_PrevFrame, _linear_clamp_sampler, uv.zw);
                    }
                    else
                    {
                        // point sample
