    ...
    public class WeatherMakerShadowMapScript : MonoBehaviour
    {
        ...
        private void CameraPreCull(Camera camera)
        {
                ...
                (cameraType == WeatherMakerCameraType.Reflection && WeatherMakerScript.Instance.PerformanceProfile.VolumetricCloudReflectionShadows))
            {
                ...
                if (WeatherMakerCommandBufferManagerScript.CameraStackCount == 1)
                {
                    int scale = (int)WeatherMakerScript.Instance.PerformanceProfile.VolumetricCloudShadowDownsampleScale;
                    tempShadowBuffer = RenderTexture.GetTemporary(WeatherMakerFullScreenEffect.GetRenderTextureDescriptor(scale, 0, 1,
        ...
        private void CameraPostRender(Camera camera)
        {
            ...
            if ((WeatherMakerCommandBufferManagerScript.CameraStackCount == 0 && cameraType == WeatherMakerCameraType.Normal) ||
                (cameraType == WeatherMakerCameraType.Reflection && WeatherMakerScript.Instance.PerformanceProfile.VolumetricCloudReflectionShadows))
            {
                if (WeatherMakerCommandBufferManagerScript.CameraStackCount == 0)
                {
